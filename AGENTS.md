...
收集区是所有素材进入知识酷的入口，后续修改必须遵守：

- 点击左侧 `收集` 后进入收集工作区。
- 收集区只围绕一个主要任务：把外部素材整理成可保存的 `原文`。
- 收集区支持的素材入口包括：
  - 文本输入。
  - 截图拖入、粘贴和上传。
  - 文件上传。
  ...
  - 媒体链接解析。
  - 网页链接解析。
- 所有入口都必须进入同一个待处理队列，队列项要明确展示：
  - 素材标题或来源。
  - 素材类型。
  - 处理状态。
  - 错误信息或禁用原因。
- 链接类素材必须先做真实解析或检查，不要只把 URL 当作普通文本假装已处理。
- 截图、文件、音视频素材必须走真实上传或真实后端解析路径，不要只做前端本地假状态。
- 收集区的主要按钮是生成 `可读原文草稿`：
  - 纯文本可以在前端整理为本地草稿。
  - 后端可处理的截图、文件、音视频、链接要调用集中 API 层。
  - API 请求必须写在 `frontend/workbench/src/api.ts`，页面不要直接散写 `fetch`。
- 生成草稿后，用户必须能编辑：
  - 标题。
  - 备注。
  - 正文 Markdown。
- 保存草稿必须真实写入 `原文库`：
  ...
  - 生成或更新对应 Markdown 文件。
  - 返回可在知识库中继续编辑的文件记录。
- 保存后的原文不做知识提炼、不直接入图谱、不自动归入重点库或视角库。
- 收集区可以提供跳转到 `学习` 或 `知识库` 的后续动作，但必须建立在真实保存结果之上。
- 禁用生成或保存按钮时，必须说明原因，例如队列为空、素材仍在上传、解析失败或草稿正文为空。
